...
for bs = 1:initNum-endValsUncy
    ...
    for n = 1:numIter
        ...
        [~,yValL(n)] = getStatsLowess( dates(tempI), wtr(tempI), mmS,...
            fitParams, R2, span);
    end
    range = prctile(yVal,[rngL 100-rngL]);
    uncy(bs,1) = meVal-range(1);
    uncy(bs,2) = meVal-range(2);
    range = prctile(yValL,[rngL 100-rngL]);
    uncy(bs,3) = meVal-range(1);
    uncy(bs,4) = meVal-range(2);
end
plot([initNum; xVal],[0; uncy(:,1)],'b-','Color',clr,'LineWidth',lW);  
plot([initNum; xVal],[0; uncy(:,2)],'b-','Color',clr,'LineWidth',lW); 
plot([initNum; xVal],[0; uncy(:,3)],'b-.','Color',clr,'LineWidth',lW);  
plot([initNum; xVal],[0; uncy(:,4)],'b-.','Color',clr,'LineWidth',lW); 
